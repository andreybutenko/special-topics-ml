...
for(i in 1:100) {
  ...
  training.indices <- seq(to = NROW(homes)) %>% sample(sample.size)
  training <- homes[training.indices,]
  testing <- homes[-training.indices,]
  
  # 2. Pass your **training data** to the `rpart` function to run a simple classification operation
  basic.fit <- rpart(in_sf ~ ., data = training, method = 'class')
  
  # 3. Pass your results to the `AssessFit` function to assess the fit
  res <- AssessFit(basic.fit)
  
  # 4. Store your assessment in the `basic.fits` vector
  basic.fits <- c(basic.fits, res)
}
